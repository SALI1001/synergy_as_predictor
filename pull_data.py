...
def convert_to_dataframe(bitcoin_data, ethereum_data, tether_data):
    ...
    tether_prices = {datetime.utcfromtimestamp(item[0] / 1000).strftime('%Y-%m-%d'): item[1] for item in tether_data}
    ...
    df = pd.DataFrame({
        ...
        'Tether': list(tether_prices.values())
    })
...
# Fetch historical data for Bitcoin, Ethereum, and Tether (last 365 days)
bitcoin_data = get_historical_data('bitcoin', 365)
ethereum_data = get_historical_data('ethereum', 365)
tether_data = get_historical_data('tether', 365)

# Convert the fetched data into a DataFrame
if bitcoin_data and ethereum_data and tether_data:
    df = convert_to_dataframe(bitcoin_data, ethereum_data, tether_data)
    print(df)
else:
...
def transform_to_binary(df):
    ...
    # Iterate over each column (Bitcoin, Ethereum, Tether)
...
def compute_pid_over_windows(df):
    ...
    for i in range(0, len(df) - 13):  # Ensure we only process full 14-day windows
        # Get the last 14 days of data for Bitcoin, Ethereum, and Tether
        bitcoin_chunk = df['Bitcoin'].iloc[i:i+14].values
        ethereum_chunk = df['Ethereum'].iloc[i:i+14].values
        tether_chunk = df['Tether'].iloc[i:i+14].values
        
        # Compute the PID values using your function
        RED, SYN, UN_x, UN_y = inf_funcs.pidMMI(bitcoin_chunk, ethereum_chunk, tether_chunk, _local=False)
...
#print(pid_df)
#pid_df.to_csv('pid_results.csv', index=False)
